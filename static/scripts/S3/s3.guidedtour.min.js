...
settings.preStepCallback(settings.$li.index(),settings.$next_tip);opts_arr=(settings.$li.data('options')||':').split(';');opts_len=opts_arr.length;for(ii=opts_len-1;ii>=0;ii--){p=opts_arr[ii].split(':');if(p.length===2){opts[$.trim(p[0])]=$.trim(p[1]);}}
settings.tipSettings=$.extend({},settings,opts);settings.tipSettings.tipLocationPattern=settings.tipLocationPatterns[settings.tipSettings.tipLocation];if(settings.modal&&settings.expose){methods.expose();}
if(!/body/i.test(settings.$target.selector)&&settings.scroll){methods.scroll_to();}
...
return(settings.$window.width()<767)?true:false;},support_localstorage:function(){if(Modernizr){return Modernizr.localstorage;}else{return!!window.localStorage;}},hide:function(){if(settings.modal&&settings.expose){methods.un_expose();}
...
if(!/body/i.test(settings.$target.selector)){if(methods.bottom()){settings.$next_tip.css({top:(settings.$target.offset().top+nub_height+settings.$target.outerHeight()),left:settings.$target.offset().left});if(/right/i.test(settings.tipSettings.nubPosition)){settings.$next_tip.css('left',settings.$target.offset().left-settings.$next_tip.outerWidth()+settings.$target.outerWidth());}
methods.nub_position($nub,settings.tipSettings.nubPosition,'top');}else if(methods.top()){settings.$next_tip.css({top:(settings.$target.offset().top-settings.$next_tip.outerHeight()-nub_height),left:settings.$target.offset().left});methods.nub_position($nub,settings.tipSettings.nubPosition,'bottom');}else if(methods.right()){settings.$next_tip.css({top:settings.$target.offset().top,left:(settings.$target.outerWidth()+settings.$target.offset().left+nub_width)});methods.nub_position($nub,settings.tipSettings.nubPosition,'left');}else if(methods.left()){settings.$next_tip.css({top:settings.$target.offset().top,left:(settings.$target.offset().left-settings.$next_tip.outerWidth()-nub_width)});methods.nub_position($nub,settings.tipSettings.nubPosition,'right');}
...
el.data('expose-css',origCSS);exposeCover.css({top:el.offset().top,left:el.offset().left,width:el.outerWidth(true),height:el.outerHeight(true)});settings.$body.append(exposeCover);expose.addClass(randId);exposeCover.addClass(randId);if(settings.tipSettings['exposeClass']){expose.addClass(settings.tipSettings['exposeClass']);exposeCover.addClass(settings.tipSettings['exposeClass']);}
el.data('expose',randId);settings.postExposeCallback(settings.$li.index(),settings.$next_tip,el);methods.add_exposed(el);},un_expose:function(){var exposeId,el,expose,origCSS,clearAll=false;if(arguments.length>0&&arguments[0]instanceof $){el=arguments[0];}else if(settings.$target&&!/body/i.test(settings.$target.selector)){el=settings.$target;}else{return false;}
if(el.length<1){if(window.console){console.error('element not valid',el);}
return false;}
...
if(settings.localStorage){localStorage.setItem(settings.localStorageKey,true);}
...
var tabbables=$(el).find(":tabbable"),first=tabbables.filter(":first"),last=tabbables.filter(":last");if(event.target===last[0]&&!event.shiftKey){first.focus(1);event.preventDefault();}else if(event.target===first[0]&&event.shiftKey){last.focus(1);event.preventDefault();}});}};$.fn.joyride=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.joyride');}};}(jQuery,this));function pre_step_callback(tip_index,tip_html){$.each(pre_step_data,function(indx,list){var cnt=list[0];var dt_id=list[1];var dt_row=list[2];if(tip_index==cnt){var columns=$('#'+dt_id+' thead th').map(function(){return $(this).text().toLowerCase();});columns[0]="id";var tableObject=$('#'+dt_id+' tbody tr').map(function(i){var row={};if(i==dt_row){$(this).find('td').each(function(i){var rowName=columns[i];if(rowName=='id'){row[rowName]=$(this).find('a').attr('db_id');}
else{row[rowName]=$(this).text();}});return row;}}).get();var html=$(tip_html)[0].innerHTML;if(tableObject.length>0){var replacementData=tableObject[0];$.each(replacementData,function(key,value){var re=new RegExp("dt_"+key,"g");html=html.replace(re,value);});}
$($(tip_html)[0]).html(html);}});}
function post_step_callback(tip_index,tip_html){$.each(post_step_data,function(indx,list){var cnt=list[0];var url=list[1];if(tip_index==cnt){if(list.length>2){var dt_id=list[2]
var dt_row=list[3];var row_id=$($('td:first a:first','#'+dt_id+' tbody tr')[dt_row]).attr('db_id')
url=url.replace(/dt_id/g,row_id);}
window.location.href=url;}});}
function post_ride_callback(tip_index,tip_html){var data=new Object();data['completed']=post_ride_data[0];data['tour_id']=post_ride_data[1];$.ajax({type:'POST',url:S3.Ap.concat('/tour/guided_tour_finished'),data:data});}
$(document).ready(function(){var pre_step_html=$('#prestep_data');if(pre_step_html.length>0){pre_step_data=$.parseJSON(pre_step_html.val());prestep_callback=pre_step_callback;}else{prestep_callback=$.noop;}
var post_step_html=$('#poststep_data');if(post_step_html.length>0){post_step_data=$.parseJSON(post_step_html.val());poststep_callback=post_step_callback;}else{poststep_callback=$.noop;}
var post_ride_html=$('#postride_data');if(post_ride_html.length>0){post_ride_data=$.parseJSON(post_ride_html.val());postride_callback=post_ride_callback;}else{postride_callback=$.noop;}
$("#joyrideID_1").joyride({autoStart:true,preStepCallback:prestep_callback,postStepCallback:poststep_callback,postRideCallback:post_ride_callback});});