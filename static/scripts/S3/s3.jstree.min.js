...
var instance_counter=0,ccp_node=false,ccp_mode=false,ccp_inst=false,themes_loaded=[],src=$('script:last').attr('src'),_d=document,_node=_d.createElement('LI'),_temp1,_temp2;_node.setAttribute('role','treeitem');_temp1=_d.createElement('I');_temp1.className='jstree-icon jstree-ocl';_temp1.setAttribute('role','presentation');_node.appendChild(_temp1);_temp1=_d.createElement('A');_temp1.className='jstree-anchor';_temp1.setAttribute('href','#');_temp1.setAttribute('tabindex','-1');_temp2=_d.createElement('I');_temp2.className='jstree-icon jstree-themeicon';_temp2.setAttribute('role','presentation');_temp1.appendChild(_temp2);_node.appendChild(_temp1);_temp1=_temp2=null;$.jstree={version:'3.0.9',defaults:{plugins:[]},plugins:{},path:src&&src.indexOf('/')!==-1?src.replace(/\/[^\/]+$/,''):'',idregex:/[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g};$.jstree.create=function(el,options){var tmp=new $.jstree.core(++instance_counter),opt=options;options=$.extend(true,{},$.jstree.defaults,options);if(opt&&opt.plugins){options.plugins=opt.plugins;}
$.each(options.plugins,function(i,k){if(i!=='core'){tmp=tmp.plugin(k,options[k]);}});tmp.init(el,options);return tmp;};$.jstree.destroy=function(){$('.jstree:jstree').jstree('destroy');$(document).off('.jstree');};$.jstree.core=function(id){this._id=id;this._cnt=0;this._wrk=null;this._data={core:{themes:{name:false,dots:false,icons:false},selected:[],last_error:{},working:false,worker_queue:[],focused:null}};};$.jstree.reference=function(needle){var tmp=null,obj=null;if(needle&&needle.id){needle=needle.id;}
...
this.each(function(){var instance=$.jstree.reference(this),method=is_method&&instance?instance[arg]:null;result=is_method&&method?method.apply(instance,args):null;if(!instance&&!is_method&&(arg===undefined||$.isPlainObject(arg))){$(this).data('jstree',new $.jstree.create(this,arg));}
...
else{if(window.getSelection){var sel=window.getSelection();try{sel.removeAllRanges();sel.collapse();}catch(ignore){}}}}).on("mousedown.jstree",$.proxy(function(e){if(e.target===this.element[0]){e.preventDefault();was_click=+(new Date());}},this)).on("mousedown.jstree",".jstree-ocl",function(e){e.preventDefault();}).on("click.jstree",".jstree-ocl",$.proxy(function(e){this.toggle_node(e.target);},this)).on("dblclick.jstree",".jstree-anchor",$.proxy(function(e){if(this.settings.core.dblclick_toggle){this.toggle_node(e.target);}},this)).on("click.jstree",".jstree-anchor",$.proxy(function(e){e.preventDefault();if(e.currentTarget!==document.activeElement){$(e.currentTarget).focus();}
this.activate_node(e.currentTarget,e);},this)).on('keydown.jstree','.jstree-anchor',$.proxy(function(e){if(e.target.tagName==="INPUT"){return true;}
...
break;case 35:e.preventDefault();this.element.find('.jstree-anchor').filter(':visible').last().focus();break;}},this)).on("load_node.jstree",$.proxy(function(e,data){if(data.status){if(data.node.id==='#'&&!this._data.core.loaded){this._data.core.loaded=true;if(this._firstChild(this.get_container_ul()[0])){this.element.attr('aria-activedescendant',this._firstChild(this.get_container_ul()[0]).id);}
this.trigger("loaded");}
if(!this._data.core.ready){setTimeout($.proxy(function(){if(!this.get_container_ul().find('.jstree-loading').length){this._data.core.ready=true;if(this._data.core.selected.length){if(this.settings.core.expand_selected_onload){var tmp=[],i,j;for(i=0,j=this._data.core.selected.length;i<j;i++){tmp=tmp.concat(this._model.data[this._data.core.selected[i]].parents);}
tmp=$.vakata.array_unique(tmp);for(i=0,j=tmp.length;i<j;i++){this.open_node(tmp[i],false,0);}}
this.trigger('changed',{'action':'ready','selected':this._data.core.selected});}
this.trigger("ready");}},this),0);}}},this)).on('keypress.jstree',$.proxy(function(e){if(e.target.tagName==="INPUT"){return true;}
...
this.element.find('.jstree-hovered').not(e.currentTarget).mouseleave();$(e.currentTarget).mouseenter();this.element.attr('tabindex','-1');},this)).on('focus.jstree',$.proxy(function(){if(+(new Date())-was_click>500&&!this._data.core.focused){was_click=0;this.get_node(this.element.attr('aria-activedescendant'),true).find('> .jstree-anchor').focus();}},this)).on('mouseenter.jstree','.jstree-anchor',$.proxy(function(e){this.hover_node(e.currentTarget);},this)).on('mouseleave.jstree','.jstree-anchor',$.proxy(function(e){this.dehover_node(e.currentTarget);},this));},unbind:function(){this.element.off('.jstree');$(document).off('.jstree-'+this._id);},trigger:function(ev,data){if(!data){data={};}
...
return obj.parentsUntil(".jstree",".jstree-node").next(".jstree-node:visible").first();},get_prev_dom:function(obj,strict){var tmp;obj=this.get_node(obj,true);if(obj[0]===this.element[0]){tmp=this.get_container_ul()[0].lastChild;while(tmp&&tmp.offsetHeight===0){tmp=this._previousSibling(tmp);}
return tmp?$(tmp):false;}
if(!obj||!obj.length){return false;}
...
r=false;}}
if(r){if(callback&&!callback.done){callback.call(this,nodes);callback.done=true;}}},load_all:function(obj,callback){if(!obj){obj='#';}
...
cb.call(this,true);},_append_json_data:function(dom,data,cb,force_processing){dom=this.get_node(dom);dom.children=[];dom.children_d=[];if(data.d){data=data.d;if(typeof data==="string"){data=JSON.parse(data);}}
...
if(p[i]===l){c=!c;}
if(c||p[i]===o||p[i]===l){this.select_node(p[i],true,false,e);}
...
delete m[obj.id];obj.id=id;m[id]=obj;return true;},get_text:function(obj){obj=this.get_node(obj);return(!obj||obj.id==='#')?false:obj.text;},set_text:function(obj,val){var t1,t2;if($.isArray(obj)){obj=obj.slice();for(t1=0,t2=obj.length;t1<t2;t1++){this.set_text(obj[t1],val);}
return true;}
obj=this.get_node(obj);if(!obj||obj.id==='#'){return false;}
...
return true;},last_error:function(){return this._data.core.last_error;},move_node:function(obj,par,pos,callback,is_loaded,skip_redraw){var t1,t2,old_par,old_pos,new_par,old_ins,is_multi,dpc,tmp,i,j,k,l,p;par=this.get_node(par);pos=pos===undefined?0:pos;if(!par){return false;}
...
if($.isArray(obj)){obj=obj.slice();for(t1=0,t2=obj.length;t1<t2;t1++){if(this.move_node(obj[t1],par,pos,callback,is_loaded,true)){par=obj[t1];pos="after";}}
...
if($.isArray(obj)){obj=obj.slice();for(t1=0,t2=obj.length;t1<t2;t1++){tmp=this.copy_node(obj[t1],par,pos,callback,is_loaded,true);if(tmp){par=tmp;pos="after";}}
this.redraw();return true;}
obj=obj&&obj.id?obj:this.get_node(obj);if(!obj||obj.id==='#'){return false;}
old_par=(obj.parent||'#').toString();new_par=(!pos.toString().match(/^(before|after)$/)||par.id==='#')?par:this.get_node(par.parent);old_ins=obj.instance?obj.instance:(this._model.data[obj.id]?this:$.jstree.reference(obj.id));is_multi=!old_ins||!old_ins._id||(this._id!==old_ins._id);if(par.id==='#'){if(pos==="before"){pos="first";}
if(pos==="after"){pos="last";}}
switch(pos){case"before":pos=$.inArray(par.id,new_par.children);break;case"after":pos=$.inArray(par.id,new_par.children)+1;break;case"inside":case"first":pos=0;break;case"last":pos=new_par.children.length;break;default:if(!pos){pos=0;}
break;}
if(pos>new_par.children.length){pos=new_par.children.length;}
...
ccp_node=false;ccp_mode=false;ccp_inst=false;},clear_buffer:function(){ccp_node=false;ccp_mode=false;ccp_inst=false;this.trigger('clear_buffer');},edit:function(obj,default_text){obj=this.get_node(obj);if(!obj){return false;}
...
h1.remove();s.replaceWith(a);s.remove();this.set_text(obj,t);if(this.rename_node(obj,$('<div></div>').text(v)[this.settings.core.force_text?'text':'html']())===false){this.set_text(obj,t);}},this),"keydown":function(event){var key=event.which;if(key===27){this.value=t;}
...
else if(icon===true){dom.removeClass('jstree-themeicon-custom '+old).css("background","").removeAttr("rel");if(old===false){this.show_icon(obj);}}
...
if(!this.settings.checkbox.tie_selection){this.element.on('model.jstree',$.proxy(function(e,data){var m=this._model.data,p=m[data.parent],dpc=data.nodes,i,j;for(i=0,j=dpc.length;i<j;i++){m[dpc[i]].state.checked=(m[dpc[i]].original&&m[dpc[i]].original.state&&m[dpc[i]].original.state.checked);if(m[dpc[i]].state.checked){this._data.checkbox.selected.push(dpc[i]);}}},this));}
...
if(s.indexOf('down')!==-1&&dom.length){dom.find('.jstree-anchor').addClass(t?'jstree-clicked':'jstree-checked').parent().attr('aria-selected',true);}},this)).on(this.settings.checkbox.tie_selection?'deselect_all.jstree':'uncheck_all.jstree',$.proxy(function(e,data){var obj=this.get_node('#'),m=this._model.data,i,j,tmp;for(i=0,j=obj.children_d.length;i<j;i++){tmp=m[obj.children_d[i]];if(tmp&&tmp.original&&tmp.original.state&&tmp.original.state.undetermined){tmp.original.state.undetermined=false;}}},this)).on(this.settings.checkbox.tie_selection?'deselect_node.jstree':'uncheck_node.jstree',$.proxy(function(e,data){var obj=data.node,dom=this.get_node(obj,true),i,j,tmp,s=this.settings.checkbox.cascade,t=this.settings.checkbox.tie_selection;if(obj&&obj.original&&obj.original.state&&obj.original.state.undetermined){obj.original.state.undetermined=false;}
...
if(this.settings.checkbox.cascade.indexOf('up')!==-1){this.element.on('delete_node.jstree',$.proxy(function(e,data){var p=this.get_node(data.parent),m=this._model.data,i,j,c,tmp,t=this.settings.checkbox.tie_selection;while(p&&p.id!=='#'){c=0;for(i=0,j=p.children.length;i<j;i++){c+=m[p.children[i]].state[t?'selected':'checked'];}
if(c===j){p.state[t?'selected':'checked']=true;this._data[t?'core':'checkbox'].selected.push(p.id);tmp=this.get_node(p,true);if(tmp&&tmp.length){tmp.attr('aria-selected',true).children('.jstree-anchor').addClass(t?'jstree-clicked':'jstree-checked');}}
else{break;}
p=this.get_node(p.parent);}},this)).on('move_node.jstree',$.proxy(function(e,data){var is_multi=data.is_multi,old_par=data.old_parent,new_par=this.get_node(data.parent),m=this._model.data,p,c,i,j,tmp,t=this.settings.checkbox.tie_selection;if(!is_multi){p=this.get_node(old_par);while(p&&p.id!=='#'){c=0;for(i=0,j=p.children.length;i<j;i++){c+=m[p.children[i]].state[t?'selected':'checked'];}
if(c===j){p.state[t?'selected':'checked']=true;this._data[t?'core':'checkbox'].selected.push(p.id);tmp=this.get_node(p,true);if(tmp&&tmp.length){tmp.attr('aria-selected',true).children('.jstree-anchor').addClass(t?'jstree-clicked':'jstree-checked');}}
else{break;}
p=this.get_node(p.parent);}}
...
p=this.get_node(p.parent);}},this));}};this._undetermined=function(){var i,j,m=this._model.data,t=this.settings.checkbox.tie_selection,s=this._data[t?'core':'checkbox'].selected,p=[],tt=this;for(i=0,j=s.length;i<j;i++){if(m[s[i]]&&m[s[i]].parents){p=p.concat(m[s[i]].parents);}}
this.element.find('.jstree-closed').not(':has(.jstree-children)').each(function(){var tmp=tt.get_node(this),tmp2;if(!tmp.state.loaded){if(tmp.original&&tmp.original.state&&tmp.original.state.undetermined&&tmp.original.state.undetermined===true){p.push(tmp.id);p=p.concat(tmp.parents);}}
...
ex=e.pageX;ey=e.pageY;cto=setTimeout(function(){$(e.currentTarget).trigger('contextmenu',true);},750);});$(document).on("context_hide.vakata.jstree",$.proxy(function(){this._data.contextmenu.visible=false;},this));};this.teardown=function(){if(this._data.contextmenu.visible){$.vakata.context.hide();}
...
vakata_context.element.find("ul").hide().end();});});}($));$.jstree.defaults.dnd={copy:true,open_timeout:500,is_draggable:true,check_while_dragging:true,always_copy:false,inside_pos:0,drag_selection:true,touch:true};$.jstree.plugins.dnd=function(options,parent){this.bind=function(){parent.bind.call(this);this.element.on('mousedown.jstree touchstart.jstree','.jstree-anchor',$.proxy(function(e){if(e.type==="touchstart"&&(!this.settings.dnd.touch||(this.settings.dnd.touch==='selected'&&!$(e.currentTarget).hasClass('jstree-clicked')))){return true;}
var obj=this.get_node(e.target),mlt=this.is_selected(obj)&&this.settings.drag_selection?this.get_selected().length:1,txt=(mlt>1?mlt+' '+this.get_string('nodes'):this.get_text(e.currentTarget));if(this.settings.core.force_text){txt=$('<div />').text(txt).html();}
if(obj&&obj.id&&obj.id!=="#"&&(e.which===1||e.type==="touchstart")&&(this.settings.dnd.is_draggable===true||($.isFunction(this.settings.dnd.is_draggable)&&this.settings.dnd.is_draggable.call(this,(mlt>1?this.get_selected(true):[obj]))))){this.element.trigger('mousedown.jstree');return $.vakata.dnd.start(e,{'jstree':true,'origin':this,'obj':this.get_node(obj,true),'nodes':mlt>1?this.get_selected():[obj.id]},'<div id="jstree-dnd" class="jstree-'+this.get_theme()+' jstree-'+this.get_theme()+'-'+this.get_theme_variant()+' '+(this.settings.core.themes.responsive?' jstree-dnd-responsive':'')+'"><i class="jstree-icon jstree-er"></i>'+txt+'<ins class="jstree-copy" style="display:none;">+</ins></div>');}},this));};};$(function(){var lastmv=false,laster=false,opento=false,marker=$('<div id="jstree-marker">&#160;</div>').hide();$(document).on('dnd_start.vakata.jstree',function(e,data){lastmv=false;if(!data||!data.data||!data.data.jstree){return;}
marker.appendTo('body');}).on('dnd_move.vakata.jstree',function(e,data){if(opento){clearTimeout(opento);}
if(!data||!data.data||!data.data.jstree){return;}
if(data.event.target.id&&data.event.target.id==='jstree-marker'){return;}
var ins=$.jstree.reference(data.event.target),ref=false,off=false,rel=false,l,t,h,p,i,o,ok,t1,t2,op,ps,pr,ip,tm;if(ins&&ins._data&&ins._data.dnd){marker.attr('class','jstree-'+ins.get_theme()+(ins.settings.core.themes.responsive?' jstree-dnd-responsive':''));data.helper.children().attr('class','jstree-'+ins.get_theme()+' jstree-'+ins.get_theme()+'-'+ins.get_theme_variant()+' '+(ins.settings.core.themes.responsive?' jstree-dnd-responsive':'')).find('.jstree-copy').first()[data.data.origin&&(data.data.origin.settings.dnd.always_copy||(data.data.origin.settings.dnd.copy&&(data.event.metaKey||data.event.ctrlKey)))?'show':'hide']();if((data.event.target===ins.element[0]||data.event.target===ins.get_container_ul()[0])&&ins.get_container_ul().children().length===0){ok=true;for(t1=0,t2=data.data.nodes.length;t1<t2;t1++){ok=ok&&ins.check((data.data.origin&&(data.data.origin.settings.dnd.always_copy||(data.data.origin.settings.dnd.copy&&(data.event.metaKey||data.event.ctrlKey)))?"copy_node":"move_node"),(data.data.origin&&data.data.origin!==ins?data.data.origin.get_node(data.data.nodes[t1]):data.data.nodes[t1]),'#','last',{'dnd':true,'ref':ins.get_node('#'),'pos':'i','is_multi':(data.data.origin&&data.data.origin!==ins),'is_foreign':(!data.data.origin)});if(!ok){break;}}
...
lastmv=false;data.helper.find('.jstree-icon').removeClass('jstree-ok').addClass('jstree-er');marker.hide();}).on('dnd_scroll.vakata.jstree',function(e,data){if(!data||!data.data||!data.data.jstree){return;}
marker.hide();lastmv=false;data.helper.find('.jstree-icon').first().removeClass('jstree-ok').addClass('jstree-er');}).on('dnd_stop.vakata.jstree',function(e,data){if(opento){clearTimeout(opento);}
if(!data||!data.data||!data.data.jstree){return;}
marker.hide().detach();var i,j,nodes=[];if(lastmv){for(i=0,j=data.data.nodes.length;i<j;i++){nodes[i]=data.data.origin?data.data.origin.get_node(data.data.nodes[i]):data.data.nodes[i];if(data.data.origin){nodes[i].instance=data.data.origin;}}
lastmv.ins[data.data.origin&&(data.data.origin.settings.dnd.always_copy||(data.data.origin.settings.dnd.copy&&(data.event.metaKey||data.event.ctrlKey)))?'copy_node':'move_node'](nodes,lastmv.par,lastmv.pos);for(i=0,j=nodes.length;i<j;i++){if(nodes[i].instance){nodes[i].instance=null;}}}
...
this._load_nodes(!$.isArray(d)?[]:$.vakata.array_unique(d),function(){this.search(str,true,show_only_matches);},true);},this));}
...
lastRd=rd;}
return{isMatch:bestLoc>=0,score:score};};return txt===true?{'search':search}:search(txt);};}($));$.jstree.defaults.sort=function(a,b){return this.get_text(a)>this.get_text(b)?1:-1;};$.jstree.plugins.sort=function(options,parent){this.bind=function(){parent.bind.call(this);this.element.on("model.jstree",$.proxy(function(e,data){this.sort(data.parent,true);},this)).on("rename_node.jstree create_node.jstree",$.proxy(function(e,data){this.sort(data.parent||data.node.parent,false);this.redraw_node(data.parent||data.node.parent,true);},this)).on("move_node.jstree copy_node.jstree",$.proxy(function(e,data){this.sort(data.parent,false);this.redraw_node(data.parent,true);},this));};this.sort=function(obj,deep){var i,j;obj=this.get_node(obj);if(obj&&obj.children&&obj.children.length){obj.children.sort($.proxy(this.settings.sort,this));if(deep){for(i=0,j=obj.children_d.length;i<j;i++){this.sort(obj.children_d[i],false);}}}};};var to=false;$.jstree.defaults.state={key:'jstree',events:'changed.jstree open_node.jstree close_node.jstree check_node.jstree uncheck_node.jstree',ttl:false,filter:false};$.jstree.plugins.state=function(options,parent){this.bind=function(){parent.bind.call(this);var bind=$.proxy(function(){this.element.on(this.settings.state.events,$.proxy(function(){if(to){clearTimeout(to);}
...
jQuery(this).jstree(c);};try{document.registerElement("vakata-jstree",{prototype:proto});}catch(ignore){}}}(jQuery));}));(function($,undefined){"use strict";var hierarchicaloptsID=0;$.widget('s3.hierarchicalopts',{options:{selected:null,multiple:true,leafonly:true,cascade:false,bulkSelect:false,cascadeOptionInTree:true,noneSelectedText:'Select',selectedText:'selected',noOptionsText:'No options available',selectAllText:'Select All',deselectAllText:'Deselect All',icons:false,stripes:true,htmlTitles:true},_create:function(){var el=$(this.element),opts=this.options;this.id=hierarchicaloptsID;hierarchicaloptsID+=1;this._namespace='.hierarchicalopts'+hierarchicaloptsID;this.treeID=el.attr('id')+'-tree';this.input=el.find('.s3-hierarchy-input').first();this.input.data('input',true);var s=opts.selected;if(s){this.input.val(this._stringifySelection(s));}
this.button=$('<button type="button" class="s3-hierarchy-button ui-multiselect ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-triangle-1-s"></span></button>');this.buttonText=$('<span>'+opts.noneSelectedText+'</span>').appendTo(this.button);this.noopts=$('<span class="no-options-available">'+opts.noOptionsText+'</span>').hide();this.tree=el.find('.s3-hierarchy-tree').first();this.wrapper=this.tree.closest('.s3-hierarchy-wrapper').hide().before(this.noopts).before(this.button).detach().appendTo('body');this._isOpen=false;this._isBulk=false;this.manualCascadeOption=false;},_init:function(){this.refresh();},_destroy:function(){$.Widget.prototype.destroy.call(this);this._unbindEvents();$(this.button).remove();$(this.noopts).remove();$(this.tree).detach().appendTo(this.element).show();},refresh:function(){this._unbindEvents();var opts=this.options,tree=this.tree,self=this;if(!tree.find('li').length){this.button.hide();this.noopts.show();return;}else{this.noopts.hide();this.button.show();}
this.input.next('.error_wrapper').insertAfter(this.button).one('click',function(){$(this).fadeOut();});var currentValue=this.input.val();if(currentValue){var selectedValues=this._parseSelection(currentValue),treeID=this.treeID;$.each(selectedValues,function(){$('#'+treeID+'-'+this).data('jstree',{selected:true});});}
var roots=tree.find('> ul > li');if(roots.length==1){var root=roots.first();var node_data=root.data('jstree');root.data('jstree',$.extend({},node_data,{'opened':true}));}
var multiple=opts.multiple,leafonly=opts.leafonly,cascade=opts.cascade,three_state=false,contextMenu=null,plugins=['sort','checkbox'];this.input.data('multiple',multiple);if((cascade||leafonly)&&multiple){three_state=true;}else if(multiple){if(!opts.cascadeOptionInTree){contextMenu=function(node){if(tree.jstree('is_parent',node)){$.vakata.context.settings.icons=false;return{'select_all':{label:self.options.selectAllText,icon:false,action:function(obj){self._selectBranch(node);}},'deselect_all':{label:self.options.deselectAllText,icon:false,action:function(obj){self._deselectBranch(node);}}};}else{return null;}};plugins.push('contextmenu');}else{this.manualCascadeOption=true;}}
tree.jstree({'core':{'themes':{name:'s3',icons:opts.icons,stripes:opts.stripes},animation:100,multiple:multiple,check_callback:true},'checkbox':{three_state:three_state},'sort':function(a,b){var sorted=this.get_text(a)>this.get_text(b)?1:-1,aRel,bRel;try{aRel=this.get_node(a).li_attr.rel;}catch(e){aRel=null;}
try{bRel=this.get_node(b).li_attr.rel;}catch(e){bRel=null;}
if(aRel=='bulk'){return-1;}else if(bRel=='bulk'){return 1;}else if(aRel=='none'){return-1;}else if(bRel=='none'){return 1;}else{return sorted;}},'contextmenu':{items:contextMenu,select_node:false,icons:false},'plugins':plugins});var inst=jQuery.jstree.reference(tree);if(multiple){if((opts.bulkSelect||this.manualCascadeOption)&&opts.cascadeOptionInTree){this.wrapper.find('.s3-hierarchy-header').hide();inst.create_node('#',{id:this.treeID+'-select-all',text:opts.selectAllText,li_attr:{'rel':'bulk','class':'s3-hierarchy-action-node'}},"first");}else{this.wrapper.find('.s3-hierarchy-header').removeClass('hide').show();}}
tree.on('ready.jstree',function(){var selected=inst.get_checked();self._updateButtonText(selected);});this._bindEvents();},_setOption:function(key,value){if(key==="selected"){var inst=jQuery.jstree.reference($(this.tree));if(inst){inst.uncheck_all();if(value){this.input.val(this._stringifySelection(value));var treeID=this.treeID;$.each(value,function(){inst.check_node('#'+treeID+'-'+this);});this._updateButtonText(value);}}}
this._super(key,value);},_updateSelectedNodes:function(){var inst=jQuery.jstree.reference($(this.tree));var oldSelected=this._parseSelection(this.input.val()),newSelected=[],selectedIDs=[],multiple=this.options.multiple,leafonly=this.options.leafonly;var nodes=inst.get_checked(true);$(nodes).each(function(){var id=$(this).attr('id');if($(this).attr('rel')=='bulk'){return;}
if(id&&(!leafonly||inst.is_leaf(this))){var record_id=id.split('-').pop();if(record_id!='None'){record_id=parseInt(record_id);}
if(record_id){newSelected.push(record_id);selectedIDs.push(id);}}});var changed=false,diff=$(newSelected).not(oldSelected).get();if(diff.length){changed=true;}else{diff=$(oldSelected).not(newSelected).get();if(diff.length){changed=true;}}
var input=this.input.val(this._stringifySelection(newSelected));this._updateButtonText(selectedIDs);if(changed){input.change();$(this.element).trigger('select.s3hierarchy');}
return true;},_updateButtonText:function(selectedIDs){var text=null,options=this.options,limit=1,numSelected=0;if(selectedIDs){numSelected=selectedIDs.length;}
if(numSelected){if(numSelected>limit){text=options.selectedText.replace('#',numSelected);}else{var items=[];for(var i=0;i<numSelected;i++){items.push($('#'+selectedIDs[i]+" > a").text().replace(/^\s+|\s+$/g,''));}
text=items.length?items.join(' '):options.noneSelectedText;}}else{text=options.noneSelectedText;}
this.buttonText.text(text);},_updateAndClose:function(){if(!this._isBulk){this._updateSelectedNodes();var wrapper=this.wrapper,self=this;if(self.options.multiple){wrapper.unbind('mouseleave.hierarchicalopts').one('mouseleave.hierarchicalopts',function(){window.setTimeout(function(){self.closeMenu();},100);});}else{window.setTimeout(function(){self.closeMenu();},100);}}},_selectNode:function(node){var opts=this.options,inst=jQuery.jstree.reference($(this.tree));var multiple=opts.multiple,leafonly=opts.leafonly;if(!multiple){var nodeID=node.id;$(inst.get_checked(true)).each(function(){if(nodeID!=this.id){if(!leafonly||inst.is_leaf(this)){inst.uncheck_node(this);}}});}else if(!this._isBulk){if(node.li_attr.rel=='bulk'){var parentID=inst.get_parent(node);if(!parentID||parentID=='#'){inst.select_all();}else{var parentNode=inst.get_node(parentID);this._selectBranch(parentNode,true);}}}
if(leafonly&&!multiple&&!inst.is_leaf(node)){inst.deselect_node(node);return false;}
this._updateAndClose();},_deselectNode:function(node){var inst=jQuery.jstree.reference($(this.tree));if(!this._isBulk){if(node.li_attr.rel=='bulk'){var parentID=inst.get_parent(node);if(!parentID||parentID=='#'){inst.deselect_all();}else{var parentNode=inst.get_node(parentID);this._deselectBranch(parentNode,true);}}else{var parentID=inst.get_parent(node);if(parentID){var bulkNode=inst.get_node(parentID+'-select-all');if(bulkNode){this._isBulk=true;inst.uncheck_node(bulkNode);this._isBulk=false;}}}
var selectAllNode=inst.get_node(this.treeID+'-select-all');if(selectAllNode){this._isBulk=true;inst.uncheck_node(selectAllNode);this._isBulk=false;}}
this._updateSelectedNodes();},_openNode:function(node){var inst=jQuery.jstree.reference($(this.tree));var parent=inst.get_parent(node);if(parent.length){inst.open_node(parent,false,true);}
if(this.manualCascadeOption){var nodeID=node.id;var bulkNode=inst.get_node(nodeID+'-select-all');if(!bulkNode){var bulkNodeID=inst.create_node(node,{id:nodeID+'-select-all',text:this.options.selectAllText,li_attr:{'rel':'bulk','class':'s3-hierarchy-action-node'}},"first");bulkNode=inst.get_node(bulkNodeID);}
var selectAllNode=inst.get_node(this.treeID+'-select-all');if(selectAllNode&&inst.is_checked(selectAllNode)){this._isBulk=true;inst.check_node(bulkNode);this._isBulk=false;}}},_selectBranch:function(node,exceptParent){var inst=jQuery.jstree.reference($(this.tree)),self=this;this._isBulk=true;inst.open_node(node,function(){var children=inst.get_children_dom(node);$(children).each(function(){self._selectBranch(this);});if(!exceptParent){inst.check_node(node);}});this._isBulk=false;this._updateAndClose();},_deselectBranch:function(node,exceptParent){var inst=jQuery.jstree.reference($(this.tree)),self=this;this._isBulk=true;inst.open_node(node,function(){var children=inst.get_children_dom(node);$(children).each(function(){self._deselectBranch(this);});if(!exceptParent){inst.uncheck_node(node);}});this._isBulk=false;this._updateSelectedNodes();},set:function(values){var inst=jQuery.jstree.reference($(this.tree)),node,self=this,treeID=this.treeID;this._isBulk=true;inst.uncheck_all();inst.close_all();if(values){var openAncestors=function(nodeID,callback){var parent=inst.get_parent(nodeID);if(parent){if(parent!='#'){openAncestors(parent);inst.open_node(parent,callback);}else if(callback){callback();}}};values.forEach(function(index){var node=inst.get_node(treeID+'-'+index);if(node){openAncestors(node,function(){inst.check_node(node);});}});}
this._isBulk=false;this._updateSelectedNodes();},get:function(){return this._parseSelection(this.input.val());},reset:function(){var inst=jQuery.jstree.reference($(this.tree));this._isBulk=true;inst.uncheck_all();inst.close_all();this._isBulk=false;this._updateSelectedNodes();return true;},_setMenuPosition:function(wrapper,button){var pos=button.offset(),css={position:'absolute',top:pos.top+button.outerHeight(),minWidth:button.outerWidth()-8};if($('body').css('direction')==='rtl'){css.right=($(window).width()-(pos.left+button.outerWidth()));}else{css.left=pos.left;}
wrapper.css(css);},openMenu:function(){if(this._isOpen){this.closeMenu();}
var button=$(this.button),wrapper=$(this.wrapper),self=this;this._setMenuPosition(wrapper,button);$(window).on('resize'+this._namespace+'-mpos',function(){self._setMenuPosition(wrapper,button);});wrapper.show();$(this.tree).jstree('set_focus');this._isOpen=true;button.addClass('ui-state-active');$(this).trigger('open');},closeMenu:function(){$(window).off(this._namespace+'-mpos');$(this.tree).jstree('unset_focus').unbind('click.hierarchicalopts').unbind('mouseleave.hierarchicalopts');$(this.wrapper).fadeOut(50);this._isOpen=false;$(this.button).removeClass('ui-state-active');$(this).trigger('close');},addNode:function(parent,id,title,check){var tree=this.tree,treeID=this.treeID,parentNode='#';if(parent){parentNode=$('#'+treeID+'-'+parent);if(!parentNode.length){parentNode='#';}}
var nodeID=treeID+'-'+id;tree.jstree('create_node',parentNode,{id:nodeID,text:title,li_attr:{rel:'leaf'}},"last");if(parent){parentNode.attr({rel:'parent'});tree.jstree('open_node',parentNode);}
if(check){tree.jstree('check_node',$('#'+nodeID));}},_parseSelection:function(value){if(!!value){var selected=JSON.parse(value);}else{return[];}
if(!!selected){if(selected.constructor!==Array){selected=[selected];}}else{selected=[];}
return selected;},_stringifySelection:function(selected){if(!this.options.multiple){if(selected.length){selected=selected[0];}else{return'';}}
return JSON.stringify(selected);},_bindEvents:function(){var self=this,wrapper=$(this.wrapper),tree=$(this.tree),button=$(this.button),ns=this._namespace,opts=this.options;var inst=jQuery.jstree.reference(tree);var icons=$.vakata.context.settings.icons;$(document).bind('context_show.vakata',function(){wrapper.unbind('mouseleave.hierarchicalopts');$.vakata.context.settings.icons=icons;});tree.bind('select_node.jstree',function(event,data){self._selectNode(data.node);}).bind('deselect_node.jstree',function(event,data){self._deselectNode(data.node);}).bind("open_node.jstree",function(event,data){self._openNode(data.node);});button.bind('click'+ns,function(){if(!self._isOpen){self.openMenu();}else{self.closeMenu();}}).bind('keyup'+ns,function(event){event.preventDefault();switch(event.keyCode){case 27:case 38:case 37:self.closeMenu();break;case 39:case 40:self.openMenu();break;}}).bind('mouseenter'+ns,function(){if(!button.hasClass('ui-state-disabled')){$(this).addClass('ui-state-hover');}}).bind('mouseleave'+ns,function(){$(this).removeClass('ui-state-hover');}).bind('focus'+ns,function(){if(!button.hasClass('ui-state-disabled')){$(this).addClass('ui-state-focus');}}).bind('blur'+ns,function(){$(this).removeClass('ui-state-focus');});if(opts.multiple&&opts.bulkSelect&&!opts.cascadeOptionInTree){wrapper.find('.s3-hierarchy-select-all').bind('click'+ns,function(event){event.preventDefault();inst.select_all();self._updateSelectedNodes();});wrapper.find('.s3-hierarchy-deselect-all').bind('click'+ns,function(event){event.preventDefault();inst.deselect_all();self._updateSelectedNodes();});wrapper.find('.s3-hierarchy-header').bind('click'+ns,function(event){event.preventDefault();event.stopPropagation();}).bind('mousedown'+ns,function(event){event.preventDefault();event.stopPropagation();});}
$(document).bind('mousedown'+ns,function(event){var target=event.target;if($('.jstree-contextmenu').has(target).length||event.which==3){wrapper.unbind('mouseleave.hierarchicalopts');return true;}
if(!tree.is(target)&&!button.is(target)&&tree.has(target).length===0&&button.has(target).length===0){self.closeMenu();}});return true;},_unbindEvents:function(){var tree=$(this.tree),button=$(this.button),ns=this._namespace;tree.unbind('select_node.jstree').unbind('deselect_node.jstree').unbind('loaded.jstree').unbind('open_node.jstree');$(this.button).unbind(ns);$(document).unbind(ns);return true;}});})(jQuery);(function($,undefined){"use strict";var hierarchicalcrudID=0;$.widget('s3.hierarchicalcrud',{options:{widgetID:null,ajaxURL:null,openLabel:'Open',openURL:null,editTitle:'Edit Record',editLabel:'Edit',editURL:null,deleteLabel:'Delete',deleteURL:null,deleteRoot:true,addTitle:'Add Record',addLabel:'Add',addURL:null,icons:false,stripes:true,htmlTitles:true},_create:function(){var el=$(this.element);this.treeID=el.attr('id')+'-tree';this.id=hierarchicalcrudID;hierarchicalcrudID+=1;},_init:function(){var el=$(this.element);this.tree=el.find('.s3-hierarchy-tree');this.refresh();},_destroy:function(){$.Widget.prototype.destroy.call(this);},refresh:function(){var tree=this.tree,opts=this.options;this._unbindEvents();var roots=tree.find('> ul > li');if(roots.length==1){var root=roots.first();var node_data=root.data('jstree');root.data('jstree',$.extend({},node_data,{'opened':true}));}
var self=this;tree.jstree({'core':{'themes':{name:'s3',icons:opts.icons,stripes:opts.stripes},animation:100,multiple:false,check_callback:true},'contextmenu':{items:function($node){var deletable=true;if(tree.jstree(true).get_parent($node)=='#'){deletable=opts.deleteRoot;}
var items={'open':{label:self.options.openLabel,action:function(obj){self._openNode($node);},separator_after:true}};if(opts.editURL){items.edit={label:self.options.editLabel,action:function(obj){self._editNode($node);},}}
if(deletable&&opts.deleteURL){items.delete={label:self.options.deleteLabel,separator_after:true,action:function(obj){self._deleteNode($node);},}}
if(opts.addURL){items.add={label:self.options.addLabel,action:function(obj){self._addNode($node);},}}
return items;},select_node:true},'plugins':['sort','contextmenu']});this._bindEvents();},_openNode:function(node){var openURL=this.options.openURL,id=node.id;if(!openURL||!id){return;}
var record_id=parseInt(id.split('-').pop());if(record_id){openURL=openURL.replace('[id]',record_id).replace('%5Bid%5D',record_id);window.location.href=openURL;}},refreshNode:function(node){var ajaxURL=this.options.ajaxURL,id=node.attr('id');if(!ajaxURL||!id){return;}
var record_id=parseInt(id.split('-').pop());if(record_id){ajaxURL=ajaxURL+'?node='+record_id;var tree=this.tree,treeID=this.treeID;$.getJSONS3(ajaxURL,function(data){if(data.label){tree.jstree('rename_node',node,data.label);}
var children=data.children;if(children){tree.jstree('open_node',node,false,false);var child,childID,childNode,added=0;for(var i=0,len=children.length;i<len;i++){child=children[i];childID=treeID+'-'+child.node;childNode=tree.jstree('get_node','#'+childID,true);if(childNode){tree.jstree('rename_node',childNode,child.label);}else{tree.jstree('create_node',node,{id:childID,text:child.label,li_attr:{rel:'leaf'}});added++;}}
if(added){tree.jstree('open_node',node);node.attr({rel:'parent'});}}});}},_editNode:function(node){var editURL=this.options.editURL,id=node.id;if(!editURL||!id){return;}
var record_id=parseInt(id.split('-').pop());if(record_id){var url=editURL.replace('[id]',record_id).replace('%5Bid%5D',record_id).split('?')[0];url+='?node='+id+'&hierarchy='+this.options.widgetID;var uid=S3.uid();var dialog=$('<iframe id="'+uid+'" src='+url+' onload="S3.popup_loaded(\''+uid+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo('body');this._openPopup(dialog,this.options.editTitle);}},_addNode:function(parent){var addURL=this.options.addURL,parent_id=parent.id;if(!addURL||!parent_id){return;}
var record_id=parseInt(parent_id.split('-').pop());if(record_id){var url=addURL.split('?')[0];url+='?node='+parent_id+'&link_to_parent='+record_id+'&hierarchy='+this.options.widgetID;var uid=S3.uid();var dialog=$('<iframe id="'+uid+'" src='+url+' onload="S3.popup_loaded(\''+uid+'\')" class="loading" marginWidth="0" marginHeight="0" frameBorder="0"></iframe>').appendTo('body');this._openPopup(dialog,this.options.addTitle);}},_deleteNode:function(node){if(!confirm(i18n.delete_confirmation)){return false;}
var deleteURL=this.options.deleteURL,id=node.id,tree=this.tree,self=this;if(!deleteURL||!id){return;}
var record_id=parseInt(id.split('-').pop());if(record_id){var url=deleteURL.replace('[id]',record_id).replace('%5Bid%5D',record_id).split('?')[0];url+='?recursive=1';$.ajaxS3({type:'POST',url:url,data:{},dataType:'JSON',success:function(data){tree.jstree('delete_node',node);},error:function(jqXHR,textStatus,errorThrown){var msg=jqXHR.responseText;if(msg[0]=="{"){try{msg=JSON.parse(jqXHR.responseText)
S3.showAlert(msg.message,'error');}catch(e){}}}});}},_openPopup:function(dialog,title){dialog.dialog({close:function(event,ui){if(self.parent){var iframe=self.parent.$('iframe.ui-dialog-content');var width=iframe.width();iframe.width(0);window.setTimeout(function(){iframe.width(width);},300);}
dialog.remove();},minHeight:480,modal:true,open:function(event,ui){$('.ui-widget-overlay').bind('click',function(){dialog.dialog('close');});},title:title,minWidth:320,closeText:''});},_bindEvents:function(){return true;},_unbindEvents:function(){return true;}});})(jQuery);